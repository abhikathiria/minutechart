// DynamicAddonInvoicePdfGenerator.cs
using System;
using System.IO;
using System.Net;
using System.Linq;
...
public static class DynamicAddonInvoicePdfGenerator
{
    ...
    public static byte[] GeneratePdf(CompanyInvoiceSetting company, AddonInvoice invoice, AppUser user, Pricing activePlan)
    {
        using (var ms = new MemoryStream())
        {
            ...
            // ---------------- Title (Centered) ----------------
            Paragraph title = new Paragraph("INVOICE (ADD-ON)", titleFont);
            ...
            decimal subtotal = invoice.Amount;
            decimal taxBase;
            decimal sgst = 0, cgst = 0;
            decimal gstPercent = company.SgstPercent + company.CgstPercent;

            if (company.ShowGst && gstPercent > 0)
            {
                // 1. Calculate the exact price before GST
                decimal preTaxExact = subtotal / (1 + gstPercent / 100m);

                // 2. Calculate the exact total tax amount required:
                decimal exactTaxTotal = subtotal - preTaxExact;

                // 3. Split the exact tax into two equal components and round the tax values.
                decimal exactTaxSplit = exactTaxTotal / 2m;

                // Set SGST and CGST equal by rounding the half tax amount
                sgst = Math.Round(exactTaxSplit, 2);
                cgst = Math.Round(exactTaxSplit, 2);

                // 4. Re-calculate the Taxable Value (Sub Total) to absorb any rounding error,
                // ensuring the final sum is exactly the original subtotal (Gross Price).
                taxBase = subtotal - (sgst + cgst);
            }
            else
            {
                // GST hidden → do NOT split, the full amount is considered the Tax Base for the line item
                taxBase = subtotal;
            }
            ...
            foreach (var col in visibleColumns)
            {
                ...
                switch (col.ColumnKey.ToLower())
                {
                    ...
                    case "details":
                        {
                            ...
                            // Plan Name in bold
                            planDetails.Add(new Chunk($"Add-on: +{invoice.Dashboards} Dashboards\n", bold));

                            planDetails.Add(new Chunk($"Add-on Validity: {invoice.StartDate:dd-MM-yyyy} to {invoice.EndDate:dd-MM-yyyy}\n", regular));

                            planDetails.Add(new Chunk($"Active Plan: {activePlan.Name}\n", regular));
                            planDetails.Add(new Chunk($"Plan Validity: {user.SubscriptionStartDate:dd-MM-yyyy} to {user.SubscriptionEndDate:dd-MM-yyyy}\n", regular));
                    ...
                    case "rate":
                        cellPhrase = new Phrase("₹ " + taxBase.ToString("N2"), bold);
                        itemsTable.AddCell(new PdfPCell(cellPhrase)
                        {
                            HorizontalAlignment = Element.ALIGN_RIGHT,
                            VerticalAlignment = Element.ALIGN_MIDDLE
            ...
            if (company.ShowAddonTermsAndConditions)
            {
            ...
            AddTotalRow(subTotalTable, "Sub Total", taxBase, bold, bold);
            ...
            if (company.ShowAddonTermsAndConditions)
            {
                ...
                string[] termsLines = company.AddonTermsAndConditions.Split('\n');
            ...
            if (company.ShowGst && company.SgstPercent > 0)
                AddTotalRow(totalsTable, $"(+) SGST: {company.SgstPercent}%", sgst, regular, regular);

            // CGST
            if (company.ShowGst && company.CgstPercent > 0)
                AddTotalRow(totalsTable, $"(+) CGST: {company.CgstPercent}%", cgst, regular, regular);
