...
namespace minutechart.Controllers.Api
{
    ...
    public class SubscriptionController : ControllerBase
    {
        ...
        private readonly IWebHostEnvironment _webHostEnvironment; // For InvoiceService
        ...
        public async Task<IActionResult> CreateOrder([FromBody] CreateOrderDto dto)
        {
            ...
            if (plan == null) 
            {
                // LOG: Failed order creation (Plan Not Found)
                await _activityLogger.LogAsync("failed to create order: plan not found", "Subscription", $"PlanID:{dto.PlanId}");
            ...
            {
                // LOG: Failed order creation (Razorpay API Error)
                await _activityLogger.LogAsync("failed to create order: razorpay api error", "Subscription", plan.Name);
            ...
            await _activityLogger.LogAsync("initiated order for plan", "Subscription", plan.Name);
        ...
        public async Task<IActionResult> VerifyPayment([FromBody] VerifyPaymentDto dto)
        {
            ...
            if (orderRecord == null) 
            {
                // LOG: Failed verification (Order Not Found)
                await _activityLogger.LogAsync("failed to verify payment: order record not found", "Payment", dto.OrderId);
            ...
            {
                // LOG: Failed verification (Invalid Signature)
                await _activityLogger.LogAsync("failed to verify payment: invalid signature", "Payment", dto.OrderId);
            ...
            var invoiceService = new InvoiceService(_db, HttpContext.RequestServices.GetRequiredService<IEmailSender>(), _webHostEnvironment);
            ...
            await _activityLogger.LogAsync("successfully purchased and activated plan", "Subscription", plan.Name);
        ...
        public async Task<IActionResult> DownloadInvoice(string orderId)
        {
            ...
            {
                // LOG: Failed download (Invoice not found)
                await _activityLogger.LogAsync("failed to download invoice: not found or PDF missing", "Invoice", orderId);
                return NotFound();
            }

            // Full path on the server
            // Use WebHostEnvironment to get the correct content root path
            var filePath = Path.Combine(_webHostEnvironment.WebRootPath, invoice.PdfPath.TrimStart('/'));

            if (!System.IO.File.Exists(filePath))
            {
                // LOG: Failed download (File missing on server)
                await _activityLogger.LogAsync("failed to download invoice: file missing on server", "Invoice", invoice.InvoiceNumber);
                return NotFound();
            }
            ...
            // LOG: Successful Invoice Download
            await _activityLogger.LogAsync("downloaded invoice", "Invoice", invoice.InvoiceNumber);
