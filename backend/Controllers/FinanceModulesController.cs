...
namespace minutechart.Controllers
{
    ...
    public class FinanceModulesController : ControllerBase
    {
        ...
        public FinanceModulesController(MinutechartDbContext db, UserManager<AppUser> userManager, DatabaseService dbService, ActivityLogger activityLogger)
        {
            _db = db;
        ...
        public async Task<IActionResult> TransferFinanceModules([FromBody] TransferModulesRequest request)
        {
            ...
            try
            {
                ...
                var sourceModules = new List<FinanceModule>();
                foreach (var id in request.ModuleIds)
                {
                    var module = await _db.FinanceModules
                ...
                var targetModules = await _db.FinanceModules
                    .Where(q => q.AppUserId == request.TargetUserId)
                    .ToListAsync();

                var duplicates = new List<FinanceModule>();
                var copied = new List<FinanceModule>();
                ...
                foreach (var sm in sourceModules)
                {
                    ...
                    var newModule = new FinanceModule
                    {
                    ...
                    _db.FinanceModules.Add(newModule);
                ...
                await _activityLogger.LogAsync(
                    $"Transferred {copied.Count} finance modules",
                    "FinanceModule",
                    sourceName,
                    targetName
                );
            ...
            {
                return StatusCode(500, new { success = false, message = "An error occurred while transferring finance modules." });
            }
        }
    }
}
