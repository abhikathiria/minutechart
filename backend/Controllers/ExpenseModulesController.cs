...
namespace minutechart.Controllers
{
    ...
    public class ExpenseModulesController : ControllerBase
    {
        ...
        public ExpenseModulesController(MinutechartDbContext db, UserManager<AppUser> userManager, DatabaseService dbService, ActivityLogger activityLogger)
        {
            _db = db;
        ...
        public async Task<IActionResult> TransferExpenseModules([FromBody] TransferModulesRequest request)
        {
            ...
            try
            {
                ...
                var sourceModules = new List<ExpenseModule>();
                foreach (var id in request.ModuleIds)
                {
                    var module = await _db.ExpenseModules
                ...
                var targetModules = await _db.ExpenseModules
                    .Where(q => q.AppUserId == request.TargetUserId)
                    .ToListAsync();

                var duplicates = new List<ExpenseModule>();
                var copied = new List<ExpenseModule>();
                ...
                foreach (var sm in sourceModules)
                {
                    ...
                    var newModule = new ExpenseModule
                    {
                    ...
                    _db.ExpenseModules.Add(newModule);
                ...
                await _activityLogger.LogAsync(
                    $"Transferred {copied.Count} expense modules",
                    "ExpenseModule",
                    sourceName,
                    targetName
                );
            ...
            {
                return StatusCode(500, new { success = false, message = "An error occurred while transferring expense modules." });
            }
        }
    }
}
