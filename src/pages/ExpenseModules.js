...
const COMPONENTS = [
    ...
    { id: "ea_line_purchase", title: "Purchase Line" },
    { id: "ea_pie_purchase", title: "Purchase Pie" },
    { id: "ea_table_purchase", title: "Purchase Table" },

    { id: "ea_line_salary", title: "Salary Line" },
    { id: "ea_pie_salary", title: "Salary Pie" },
    { id: "ea_table_salary", title: "Salary Table" },

    { id: "ea_line_direct", title: "Direct Line" },
    { id: "ea_pie_direct", title: "Direct Pie" },
    { id: "ea_table_direct", title: "Direct Table" },

    { id: "ea_line_indirect", title: "Indirect Line" },
    { id: "ea_pie_indirect", title: "Indirect Pie" },
    { id: "ea_table_indirect", title: "Indirect Table" },

    { id: "ea_line_admin", title: "Admin Line" },
    { id: "ea_pie_admin", title: "Admin Pie" },
    { id: "ea_table_admin", title: "Admin Table" }
];
...
export default function ExpenseModules() {
    ...
    async function loadList() {
        ...
        setMessage(null);
        try {
            const res = await api.get(`/expensemodules/list/${encodeURIComponent(userId)}`);
            ...
            console.error("Failed to load expense modules list", err);
            setConfigMap({});
    ...
    async function handleTest() {
        ...
        try {
            ...
            const res = await api.post(
                `/expensemodules/test-raw/${encodeURIComponent(userId)}`,
                body
            );
    ...
    async function handleSave() {
        ...
        try {
            ...
            const res = await api.post(`/expensemodules/save/${encodeURIComponent(userId)}`, payload);
            if (res.data?.success) {
                await loadList();
    ...
    async function handleTransferCheck() {
        ...
        try {
            const res = await api.post("/expensemodules/transfer-modules", {
                sourceUserId: userId,
    ...
    async function handleToggleHide() {
        ...
        try {
            const id = cfg.id;
            const res = await api.post(`/expensemodules/toggle-hide/${id}`, { hide: !cfg.hideQuery });
            if (res.data?.success) {
    ...
    async function handleDelete() {
        ...
        setActionLoading(true);
        try {
            const res = await api.delete(`/expensemodules/delete/${moduleToDelete.cfg.id}`);
            if (res.data?.success) {
                const updated = { ...configMap };
        ...
        try {
            for (const compId of selectedModules) {
                ...
                try {
                    ...
                    await api.post(`/expensemodules/toggle-hide/${cfg.id}`, { hide: makeHide });
                    // update local map
                    setConfigMap(prev => {
                        const copy = { ...prev };
    ...
    const batchDelete = async () => {
        ...
        try {
            for (const compId of selectedModules) {
                ...
                try {
                    await api.delete(`/expensemodules/delete/${cfg.id}`);
                    setConfigMap(prev => {
                        const copy = { ...prev };
                        copy[compId] = null;
                        return copy;
    ...
    const handleToggleHideSingle = async (compId) => {
        ...
        setActionLoading(true);
        try {
            const res = await api.post(`/expensemodules/toggle-hide/${cfg.id}`, { hide: !cfg.hideQuery });
            if (res.data?.success) {
                setConfigMap(prev => {
                    const copy = { ...prev };
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 max-w-7xl mx-auto">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-800 flex items-center gap-2">
                            <FaCog className="text-indigo-600" /> Expense Modules for <span className="text-indigo-600">{companyName}</span>
                        </h1>
                ...
                <section className="flex-1 lg:w-4/12 flex flex-col space-y-6">
                    <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200 flex flex-col">
                        <div className="flex justify-between items-center mb-4 border-b pb-3">
                            <h2 className="text-xl font-bold text-gray-700">Expense Modules ({COMPONENTS.length})</h2>
            ...
            {transferModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-lg">
                        ...
                        <h2 className="text-xl font-bold mb-4">Transfer Expense Modules</h2>
