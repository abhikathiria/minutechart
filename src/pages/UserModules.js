...
import api from "../api";
import { FaRegCopy, FaCheck, FaCog, FaUsers, FaTrashAlt, FaPlus, FaChevronDown, FaChevronUp, FaSearch } from "react-icons/fa";
...
export default function UserModules() {
    ...
    const handleTransferCheck = async () => {
        try {
            ...
            // BLOCKED BY LIMIT
            if (res.data.capacityOk === false) {
                showMessages(
                    "error",
                    `Transfer blocked. Target user has reached their module limit (${res.data.dashboardLimit}).`
                );
                return;
            }
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 max-w-7xl mx-auto">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-800 flex items-center gap-2">
                            <FaCog className="text-indigo-600" /> User Modules for <span className="text-indigo-600">{companyName}</span>
                        </h1>
