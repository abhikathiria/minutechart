...
export default function Dashboard() {
  ...
  useEffect(() => {
    const fetchUserId = async () => {
      try {
        const res = await api.get("/account/me"); // Assuming this returns { id: "user-id" }
        setUserId(res.data.id);
      } catch (err) {
        console.error("Failed to fetch user ID", err);
        // Fallback: If no API, get from localStorage or context (e.g., setUserId(localStorage.getItem("userId")))
      }
    };
    fetchUserId();
        ...
        {queries.length === 0 ? (
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-2 gap-6">
            {queries.map((q) => {
              ...
              const numColumns = data.length > 0 ? Object.keys(data[0]).length : 0;
              let spanClasses = "";
              ...
              if (q.visualizationType === "table" || q.visualizationType === "heatmap") {
                if (numRows > 10 && numColumns > 5) {
                  spanClasses = "row-span-2 col-span-2";
              ...
              return (
                <div
                  key={q.userQueryId}
                  className={`bg-white rounded-xl shadow-lg p-4 flex flex-col hover:shadow-xl transition overflow-hidden ${spanClasses}`}
                >
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-semibold text-lg sm:text-xl">
                      {q.userTitle || "Untitled Module"}
