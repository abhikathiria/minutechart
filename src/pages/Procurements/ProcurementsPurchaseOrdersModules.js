...
export default function ProcurementsPurchaseOrdersModules() {
    const { id } = useParams();
    ...
    const loadUserAndModules = async () => {
        setLoadingList(true);
        try {
            const modulesRes = await api.get(`/procurements/purchase-orders/user/${id}/queries`);
    ...
    const handleDeleteConfirm = async () => {
        ...
        try {
            await api.delete(`/procurements/purchase-orders/delete-query/${moduleToDelete.id}`);
    ...
    const handleExecute = async () => {
        ...
        try {
            const res = await api.post(
                `/procurements/purchase-orders/execute-user-query/${id}`,
    ...
    const handleSave = async () => {
        ...
        try {
            ...
            const saveRes = await api.post(
                `/procurements/purchase-orders/save-user-query/${id}`,
                payload,
                { headers: { "Content-Type": "application/json" } }
            );
    ...
    const handleTransferCheck = async () => {
        ...
        try {
            const res = await api.post("/procurements/purchase-orders/transfer-modules", {
    ...
    const handleBatchDelete = async () => {
        ...
        for (const id of selectedModules) {
            try {
                await api.delete(`/procurements/purchase-orders/delete-query/${id}`);
            } catch (err) {
                failures.push(id);
            }
        }
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 max-w-7xl mx-auto">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-800 flex items-center gap-2">
                            <FaCog className="text-indigo-600" /> Procurements Purchase Orders Modules for <span className="text-indigo-600">{companyName}</span>
                        </h1>
