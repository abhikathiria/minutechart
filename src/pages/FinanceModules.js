...
const COMPONENTS = [
    ...
    { id: "fa_line_debttoequity", title: "Debt to Equity Line" },
    { id: "fa_bar_debtandequity", title: "Debt & Equity Bar" },
    { id: "fa_table_debt", title: "Debtors Table" },

    { id: "fa_line_cashturnoverratio", title: "Cash Turnover Line" },
    { id: "fa_bar_salesvscashandbank", title: "Sales vs Cash & Bank Bar" },
    { id: "fa_table_cashandbank", title: "Cash & Bank Table" },

    { id: "fa_line_faturnoverratio", title: "Fixed Assets Turnover Line" },
    { id: "fa_bar_salesvsfixedassets", title: "Sales vs Fixed Assets Bar" },
    { id: "fa_table_fixedassets", title: "Fixed Assets Table" },

    { id: "fa_line_debtors", title: "Debtors Line" },
    { id: "fa_bar_debtorsvssales", title: "Debt vs Sales Bar" },
    { id: "fa_table_debtors", title: "Debtors Table" },

    { id: "fa_line_creditors", title: "Creditors Line" },
    { id: "fa_bar_creditorsvspurchase", title: "Credit vs Purchase Bar" },
    { id: "fa_table_creditors", title: "Creditors Table" },

    { id: "fa_line_commission", title: "Commission Line" },
    { id: "fa_bar_commissionandcreditors", title: "Commission & Creditors Bar" },
    { id: "fa_table_creditorscommission", title: "Creditors Commission Table" },
];
...
export default function FinanceModules() {
    ...
    async function loadList() {
        ...
        setMessage(null);
        try {
            const res = await api.get(`/financemodules/list/${encodeURIComponent(userId)}`);
            ...
            console.error("Failed to load finance modules list", err);
            setConfigMap({});
    ...
    async function handleTest() {
        ...
        try {
            ...
            const res = await api.post(
                `/financemodules/test-raw/${encodeURIComponent(userId)}`,
                body
            );
    ...
    async function handleSave() {
        ...
        try {
            ...
            const res = await api.post(`/financemodules/save/${encodeURIComponent(userId)}`, payload);
            if (res.data?.success) {
                await loadList();
    ...
    async function handleTransferCheck() {
        ...
        try {
            const res = await api.post("/financemodules/transfer-modules", {
                sourceUserId: userId,
    ...
    async function handleToggleHide() {
        ...
        try {
            const id = cfg.id;
            const res = await api.post(`/financemodules/toggle-hide/${id}`, { hide: !cfg.hideQuery });
            if (res.data?.success) {
    ...
    async function handleDelete() {
        ...
        setActionLoading(true);
        try {
            const res = await api.delete(`/financemodules/delete/${moduleToDelete.cfg.id}`);
            if (res.data?.success) {
                const updated = { ...configMap };
        ...
        try {
            for (const compId of selectedModules) {
                ...
                try {
                    ...
                    await api.post(`/financemodules/toggle-hide/${cfg.id}`, { hide: makeHide });
                    // update local map
                    setConfigMap(prev => {
                        const copy = { ...prev };
    ...
    const batchDelete = async () => {
        ...
        try {
            for (const compId of selectedModules) {
                ...
                try {
                    await api.delete(`/financemodules/delete/${cfg.id}`);
                    setConfigMap(prev => {
                        const copy = { ...prev };
                        copy[compId] = null;
                        return copy;
    ...
    const handleToggleHideSingle = async (compId) => {
        ...
        setActionLoading(true);
        try {
            const res = await api.post(`/financemodules/toggle-hide/${cfg.id}`, { hide: !cfg.hideQuery });
            if (res.data?.success) {
                setConfigMap(prev => {
                    const copy = { ...prev };
                ...
                <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-3 max-w-7xl mx-auto">
                    <div>
                        <h1 className="text-2xl sm:text-3xl font-extrabold text-gray-800 flex items-center gap-2">
                            <FaCog className="text-indigo-600" /> Financial Modules for <span className="text-indigo-600">{companyName}</span>
                        </h1>
                ...
                <section className="flex-1 lg:w-4/12 flex flex-col space-y-6">
                    <div className="bg-white p-6 rounded-xl shadow-lg border border-gray-200 flex flex-col">
                        <div className="flex justify-between items-center mb-4 border-b pb-3">
                            <h2 className="text-xl font-bold text-gray-700">Financial Modules ({COMPONENTS.length})</h2>
            ...
            {transferModalOpen && (
                <div className="fixed inset-0 bg-black bg-opacity-40 flex items-center justify-center z-50">
                    <div className="bg-white p-6 rounded-xl shadow-xl w-full max-w-lg">
                        ...
                        <h2 className="text-xl font-bold mb-4">Transfer Finance Modules</h2>
