...
//       const [statusRes, planRes, addonRes] = await Promise.all([
...
//       setPlan(planRes.data.hasPlan ? planRes.data : null);
//       setAddons(addonRes.data || []);
//     } catch (err) {
//       setToast({
//         type: "error",
//         text: err?.response?.data?.message || "Failed to load subscription data",
//       });
//     } finally {
//       setLoading(false);
//     }
...
//   const totalAddonDash = addons.reduce((sum, a) => sum + a.Dashboards, 0);
//   const baseLimit = plan?.dashboardLimit ?? 0;
//   const totalDashboards = baseLimit + totalAddonDash;

//   const addonDashboards = plan?.addonDashboards ?? 0;
//   const addonPrice = plan?.addonPrice ?? 0;
//   const canBuyAddon = plan?.dashboardAddonEnabled === true && addonDashboards > 0;
...
//           setToast({
//             type: "success",
//             text: "Add-on purchased successfully!",
//           });
...
//           Upgrade your dashboard capacity with a single click.
//         </p>
//       </motion.div>
...
//           className="md:col-span-2 bg-white shadow-xl rounded-2xl p-6 border border-gray-100"
//         >
//           <h2 className="text-xl font-bold text-gray-900 mb-2">Active Plan</h2>
//           <p className="text-gray-500 mb-6">{plan?.name}</p>

//           <div className="grid grid-cols-2 gap-4">
//             <InfoCard label="Expires" value={status?.subscriptionEnd ? new Date(status.subscriptionEnd).toLocaleDateString() : "—"} />
//             <InfoCard label="Base Dashboards" value={baseLimit} />
//             <InfoCard label="Add-ons" value={`+${totalAddonDash}`} />
//             <InfoCard label="Total Dashboards" value={totalDashboards} />
//             <InfoCard label="Refresh Rate" value={`${plan?.refreshRateMinutes} min`} full />
//             <InfoCard label="Excel Export" value={plan?.excelExport ? "Enabled" : "Disabled"} full />
//             <InfoCard label="Addon Feature" value={plan?.dashboardAddonEnabled ? "Available" : "Not available"} full />
//           </div>
...
//               whileHover={{ scale: canBuyAddon ? 1.03 : 1 }}
//               whileTap={{ scale: 0.97 }}
...
//               Add-on Pack: {addonDashboards} dashboards for ₹{addonPrice}
//             </p>
//           </div>
//         </motion.div>
...
//         >
//           <h3 className="text-lg font-bold mb-4">Active Add-ons</h3>

//           {addons.length === 0 && (
//             <p className="text-gray-400">No add-ons purchased yet.</p>
//           )}
...
//                 Add <strong>{addonDashboards}</strong> dashboards for{" "}
//                 <strong>₹{addonPrice}</strong> (valid 30 days).
//               </p>
...
//                 >
//                   {isBuying ? "Processing..." : "Buy Add-on"}
//                 </button>
//               </div>
//             </motion.div>
//           </motion.div>
...
//               Verifying Payment & Emailing Invoice...
//             </p>
//           </motion.div>
//         )}
//       </AnimatePresence>
...
//       {toast && (
//         <div
//           className={`fixed bottom-6 right-6 px-4 py-3 rounded-xl text-white shadow-lg ${
          ...
          <div className="rounded-2xl p-4 border border-white/6">
            {loading ? (
              <div className="py-16 flex items-center justify-center">
                <FaSpinner className="animate-spin mr-3 text-white/60" />
