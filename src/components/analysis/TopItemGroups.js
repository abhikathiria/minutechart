...
import { FaBoxOpen, FaCrown } from "react-icons/fa";
import {
  PieChart,
  Pie,
  Cell,
...
const COLORS = ["#FBCF00", "#423C2E", "#822513", "#D3974E", "#C084FC", "#E9D5FF"];

function TopItemGroups({ className = "", data }) {
  ...
  const topItemGroupName = sortedData[0]?.name;
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      const itemGroup = payload[0].payload;
      const isTop = itemGroup.name === topItemGroupName;
      ...
      return (
        <div className="bg-white border border-gray-300 rounded shadow px-3 py-2 text-sm">
          ...
          <div className="text-lg font-semibold">
            Sales: ₹{itemGroup.value.toLocaleString("en-IN")}
          </div>
        </div>
      );
    }
    ...
    >
      <h2 className="text-xl font-semibold text-center p-4 text-blue-700 flex items-center justify-center space-x-2" style={{ color: headingTextColor }}>
        <span className="p-2 rounded-full" style={gradientStyle}>
          <FaBoxOpen className="text-white text-xl sm:text-2xl" />
        </span>
        <span>Top Item Groups by Sales</span>
      </h2>
