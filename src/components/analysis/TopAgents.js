...
import { FaUserSecret, FaCrown } from "react-icons/fa";
import {
  PieChart,
  Pie,
  Cell,
...
const COLORS = ["#57167E", "#9B3192", "#EA5F89", "#2B0B3F", "#6366F1", "#E9D5FF"];

function TopAgents({ className = "", data }) {
  ...
  const topAgentName = sortedData[0]?.name;
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const isTop = agent.name === topAgentName;
      ...
      return (
        <div className="bg-white border border-gray-300 rounded shadow px-3 py-2 text-sm">
          ...
          <div className="text-lg font-semibold">
            Sales: ₹{agent.value.toLocaleString("en-IN")}
          </div>
        </div>
      );
    }
  ...
  return (
    ...
    >
      <h2
        className="text-xl font-semibold text-center p-4 text-blue-700 flex items-center justify-center space-x-2"
        style={{ color: headingTextColor }}
      >
        <span className="p-2 rounded-full" style={gradientStyle}>
          <FaUserSecret className="text-white text-xl sm:text-2xl" />
        </span>
        <span>Top Agent by Sales</span>
      </h2>
