...
function TopCreditors({ className = "", data }) {
  ...
  const topCreditorName = sortedData[0]?.name;
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      const creditor = payload[0].payload;
      const isTop = creditor.name === topCreditorName;
      ...
      return (
        <div className="bg-white border border-gray-300 rounded shadow px-3 py-2 text-sm">
          ...
          <div className="text-lg font-semibold">
            Sales: ₹{creditor.value.toLocaleString("en-IN")}
          </div>
        </div>
      );
    }
