...
import { FaLayerGroup, FaCrown } from "react-icons/fa";
import {
  PieChart,
  Pie,
  Cell,
...
const COLORS = ["#3A3873", "#155D81", "#5E784A", "#ED7164", "#F2C16D", "#E9D5FF"];

function TopItems({ className = "", data }) {
  ...
  const topItemName = sortedData[0]?.name;
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const isTop = item.name === topItemName;
      ...
      return (
        <div className="bg-white border border-gray-300 rounded shadow px-3 py-2 text-sm">
          ...
          <div className="text-lg font-semibold">
            Sales: ₹{item.value.toLocaleString("en-IN")}
          </div>
        </div>
      );
    }
