...
function TopBrokers({ className = "", data }) {
  ...
  const topBrokerName = sortedData[0]?.name;
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      const broker = payload[0].payload;
      const isTop = broker.name === topBrokerName;
      ...
      return (
        <div className="bg-white border border-gray-300 rounded shadow px-3 py-2 text-sm">
          ...
          <div className="text-lg font-semibold">
            Sales: ₹{broker.value.toLocaleString("en-IN")}
          </div>
        </div>
      );
    }
