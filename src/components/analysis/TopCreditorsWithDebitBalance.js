...
function TopCreditorsWithDebitBalance({ className = "", data }) {
  ...
  const topCreditorWithDebitBalanceName = sortedData[0]?.name;
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      const creditorWithDebitBalance = payload[0].payload;
      const isTop = creditorWithDebitBalance.name === topCreditorWithDebitBalanceName;
      ...
      return (
        <div className="bg-white border border-gray-300 rounded shadow px-3 py-2 text-sm">
          <div className="flex items-center gap-1  text-2xl font-bold text-gray-800">
            ...
            <span>{creditorWithDebitBalance.name}</span>
          </div>
          <div className="text-lg font-semibold">
            Sales: ₹{creditorWithDebitBalance.value.toLocaleString("en-IN")}
          </div>
        </div>
      );
    }
    ...
    >
      <h2 className="text-xl font-semibold text-center p-4 text-blue-700 flex items-center justify-center space-x-2" style={{ color: headingTextColor }}>
        <span className="p-2 rounded-full" style={gradientStyle}>
          <MdSwapHoriz className="text-white text-xl sm:text-2xl" />
        </span>
        <span>Top Creditors With Debit Balance</span>
      </h2>
...
export default TopCreditorsWithDebitBalance;
