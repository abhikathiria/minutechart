...
import { FaTruckLoading, FaCrown } from "react-icons/fa";
import {
  PieChart,
  Pie,
  Cell,
...
const COLORS = ["#581816", "#930002", "#121B22", "#1E4969", "#C5AE97", "#E9D5FF"];

function TopSuppliers({ className = "", data }) {
  ...
  const topSupplierName = sortedData[0]?.name;
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      ...
      const isTop = supplier.name === topSupplierName;
      ...
      return (
        <div className="bg-white border border-gray-300 rounded shadow px-3 py-2 text-sm">
          ...
          <div className="text-lg font-semibold">
            Sales: ₹{supplier.value.toLocaleString("en-IN")}
          </div>
        </div>
      );
    }
    ...
    >
      <h2 className="text-xl font-semibold text-center p-4 text-blue-700 flex items-center justify-center space-x-2" style={{ color: headingTextColor }}>
        <span className="p-2 rounded-full" style={gradientStyle}>
          <FaTruckLoading className="text-white text-xl sm:text-2xl" />
        </span>
        <span>Top Suppliers by Purchases</span>
      </h2>
