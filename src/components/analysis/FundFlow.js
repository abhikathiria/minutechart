...
function FundFlow({ className = "", data }) {
  ...
  const inflow = data?.fundIn || data?.FundIn || 0;
  const outflow = data?.fundOut || data?.FundOut || 0;

  const chartData = [
    { name: "Fund In", value: inflow },
    { name: "Fund Out", value: outflow },
  ];
  ...
  return (
    ...
    >
      <h2 className="text-xl font-semibold text-center p-4 text-blue-700 flex items-center justify-center space-x-2">
        ...
        >
          <FaExchangeAlt className="text-white text-xl sm:text-2xl" />
        </span>
        <span>Fund Flow</span>
      </h2>
