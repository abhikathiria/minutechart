...
function TopDebtors({ className = "", data }) {
  ...
  const topDebtorName = sortedData[0]?.name;
  ...
  const CustomTooltip = ({ active, payload }) => {
    if (active && payload && payload.length) {
      const debtor = payload[0].payload;
      const isTop = debtor.name === topDebtorName;
      ...
      return (
        <div className="bg-white border border-gray-300 rounded shadow px-3 py-2 text-sm">
          ...
          <div className="text-lg font-semibold">
            Sales: ₹{debtor.value.toLocaleString("en-IN")}
          </div>
        </div>
      );
    }
